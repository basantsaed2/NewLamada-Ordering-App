...
const PaymentMethods = () => {
       ...
       useEffect(() => {
              dispatch(UpdateOrder({ ...order, payment_method_id: selectedPaymentMethod, paymentMethodName:selectedPaymentMethodName, receipt: reciptFile }))
              ...
              <>
                     <div className="w-full flex sm:flex-col xl:flex-row items-center justify-between border-mainColor border-[3px] rounded-2xl p-3">
                            <div className="sm:w-full xl:w-6/12">
                                   <p className="mb-2 text-3xl text-secoundColor font-TextFontRegular">
                                          {t("ChooseYourPreferred")} <span className="text-mainColor">{t("PaymentMethod")}</span>
                                   </p>
                                   <div className="">
                                          ...
                                          {CreditCards.map((card, index) => (
                                                 ...
                                                 >
                                                        ...
                                                        />
                                                        <img src={card.logo_link} alt={card.name} className='object-contain w-16 h-16 rounded-full' />
                                                        <label
                                                               htmlFor={card.id}
                                                               className="text-2xl text-secoundColor font-TextFontRegular"
                                                        >
                                                               {card.name}
                                                        </label>
                                                 </div>
                            ...
                            {(selectedPaymentMethod === 2) && (
                                   ...
                                   <div className="sm:w-full xl:w-6/12">
                                          <div className="flex justify-center gap-0 sm:w-full xl:w-3/4 sm:flex-col xl:flex-row sm:items-start xl:items-center">
                                                 <span className="w-7/12 text-2xl font-TextFontRegular text-mainColor xl:mt-2">{t("ReciptUpload")}:</span>
                                                 <UploadInput
                                                        value={recipt}
